...
export function RequireScope({ scope, children }: RequireScopeProps) {
  ...
  if (currentScope !== scope) {
    return <ForbiddenState description="Esta vista solo está disponible para otro ámbito de usuario." />;
  }
...
export function RequireMFA({ children, mfaCompleted }: RequireMFAProps) {
  if (!mfaCompleted) {
    return <ForbiddenState description="Completa el segundo factor para continuar." />;
  }
  return <>{children}</>;
}
